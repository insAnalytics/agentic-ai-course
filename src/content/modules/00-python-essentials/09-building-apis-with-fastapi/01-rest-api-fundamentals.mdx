...
<Subsection title="Resources, URIs, and the data format" variant="a">
...
— [exactly the format covered in the I/O lesson](/00-python-essentials/06-files-errors-and-validation/02-working-with-json/)
— a request body and a response body are both typically just JSON text,
parsed and serialized the same way you've already practiced.

</Subsection>

<Subsection title="The five core HTTP methods" variant="b">
...
<Subsection title="REST's guiding principles" variant="a">
...
<QuizGroup
  ...
  questions={[
    {
      question: "In REST, what does a URI like /agents/42 actually identify?",
      options: [
        "An action to perform",
        "A specific resource — in this case, the agent with id 42",
        "A database table name",
        "A function to call",
      ],
      correctIndex: 1,
      explanation:
        "REST URIs name things, not actions — /agents/42 identifies one specific agent resource, and the HTTP method says what to do with it.",
    },
    {
      question: "What does \"idempotent\" mean for an HTTP method?",
      options: [
        "The method can only be called once, ever",
        "Making the same request multiple times has the same effect as making it once — repeating it doesn't change the outcome further",
        "The method never modifies any data",
        "The method always returns the exact same response body every time",
      ],
      correctIndex: 1,
      explanation:
        "Idempotency is about the end state after repeating a request, not about the method being read-only or returning identical output — DELETE stays idempotent even though only the first call actually deletes anything.",
    },
    {
      question: "Why is creating a new resource typically done with POST rather than PUT?",
      options: [
        "POST is faster than PUT",
        "PUT promises idempotency — repeating it leaves things as they'd be after one call — while POST makes no such promise, and calling it twice typically creates two separate resources",
        "PUT can't include a request body",
        "There's no real difference; either would work identically",
      ],
      correctIndex: 1,
      explanation:
        "PUT's idempotency guarantee is exactly what creation can't offer — calling POST /agents twice with the same body should create two distinct agents, not silently no-op the second time.",
    },
    {
      question: "What's the difference between PUT and PATCH?",
      options: [
        "They're interchangeable",
        "PUT replaces a resource entirely; PATCH updates only the specific fields included in the request, leaving the rest untouched",
        "PATCH can only be used on collections, never a single resource",
        "PUT is read-only",
      ],
      correctIndex: 1,
      explanation:
        "PUT /agents/42 with a full object replaces everything about that agent; PATCH /agents/42 with just one field changes only that field, leaving the rest of the resource exactly as it was.",
    },
    {
      question: "What does \"statelessness\" mean as a REST principle?",
      options: [
        "The server never stores any data at all",
        "Every request carries everything the server needs to process it — the server doesn't remember anything about a client between one request and the next",
        "Resources can never change state",
        "The client can't send any data to the server",
      ],
      correctIndex: 1,
      explanation:
        "Statelessness is about the server not tracking session context between requests — any needed proof (a token, a header) travels with every request instead of being recalled from an earlier one.",
    },
    {
      question: "Why does REST favor resource-oriented URIs like /agents/42 with DELETE, over action-oriented ones like /deleteAgent?id=42?",
      options: [
        "Action-oriented URIs are technically invalid HTTP",
        "The URI should name a resource; the HTTP method itself already expresses the action, keeping the interface uniform and predictable across every resource",
        "/deleteAgent?id=42 is slower to process",
        "There's no meaningful difference between the two approaches",
      ],
      correctIndex: 1,
      explanation:
        "Baking the verb into the path duplicates what the HTTP method already says, and breaks the uniform interface — a client that understands GET/POST/DELETE on one resource should be able to predict how every other resource behaves too.",
    },
  ]}
/>
